...
- **Obsidian API 相容性**：透過移除對新版設定 API 的依賴，恢復與較舊 Obsidian 版本（最低至 `v1.6.6+`）的相容性。
- **同步儀表板 (Dashboard)**：新增完整的同步資訊面板，可一目了然狀態、範圍統計、上傳歷史、失敗次數，並提供快速維護操作。
- **彈出視窗相容**：將程式碼中的 `globalThis` 全面改為 `window`，修復了在彈出視窗（popout window）下的相容性警告。
- **失敗重試掃描**：可在儀表板或失敗細節彈出視窗中，直接針對有失敗紀錄的筆記重新觸發掃描與同步。
