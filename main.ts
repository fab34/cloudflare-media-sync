...
const TEXT = {
  en: {
    ...
    deleteEnabledNotice: "Local file handling is enabled. For multi-device sync, review folder mode is the safest option.",
    ...
    deleteLocalName: "Handle local image after upload",
    deleteLocalDesc: "Off by default. When enabled, successfully uploaded local images are moved according to the cleanup mode below.",
    cleanupModeName: "Local image handling mode",
    cleanupModeDesc: "Choose what happens after upload and link rewrite. Obsidian trash removes the image from the vault. Review folder keeps a temporary safety copy for manual cleanup.",
    cleanupMoveToTrash: "Move to Obsidian trash (remove from vault)",
    cleanupMoveToFolder: "Move to review folder (temporary safety copy)",
    cleanupFolderName: "Review folder",
    cleanupFolderDesc: "Vault-relative folder used as a temporary holding area. Files here are not deleted automatically; clear them from the dashboard after checking other devices.",
  ...
  "zh-TW": {
    ...
    deleteEnabledNotice: "已啟用本機圖片後續處理。多裝置同步時，建議使用檢查資料夾模式。",
    ...
    deleteLocalName: "上傳後處理本機圖片",
    deleteLocalDesc: "預設關閉。開啟後，成功上傳並改寫連結的本機圖片，會依照下方清理方式移動。",
    cleanupModeName: "本機圖片處理方式",
    cleanupModeDesc: "選擇成功上傳並改寫連結後，要如何處理本機圖片。Obsidian 垃圾桶會把圖片移出 vault；檢查資料夾會保留一份安全暫存，需之後手動清理。",
    cleanupMoveToTrash: "移到 Obsidian 垃圾桶（移出 vault）",
    cleanupMoveToFolder: "移到檢查資料夾（安全暫存）",
    cleanupFolderName: "檢查資料夾",
    cleanupFolderDesc: "作為安全暫存區使用的 vault 相對資料夾。這裡的檔案不會自動刪除；確認其他裝置正常顯示後，請從 Dashboard 清理。",
...
export default class R2MediaSyncPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.registerEvent(this.app.workspace.on("editor-paste", (event: ClipboardEvent, _editor: unknown, view: MarkdownView) => {
      if (event.defaultPrevented) return;
      if (!this.settings.processEditorPaste) return;
      this.enqueueEditorImageUpload(view);
      event.preventDefault();
    }));
    this.registerEvent(this.app.workspace.on("editor-drop", (event: DragEvent, _editor: unknown, view: MarkdownView) => {
      if (event.defaultPrevented) return;
      if (!this.settings.processEditorDrop) return;
      this.enqueueEditorImageUpload(view);
      event.preventDefault();
    }));
